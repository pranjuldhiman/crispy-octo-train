...
class SplashViewModel (private val context: Context): ViewModel() {
    ...
    fun getLogin(mobileno:String,token:String) {
        ...
        viewModelScope.launch {
            Log.e("Response","updateFcm response mobno...."+mobileno.toString())

            when(val serviceResult =
                roundUpRepository.updateFcm(mobileno.toString(),token.toString())){
    ...
    private fun onFailure(exception: String) {
        ...
        Log.e("Response", "updateFcm exception is....."+exception.toString())
    ...
    private fun onSuccessResponse(data: MODSafetyModel?) {
        data?.let {
            ...
            if (data.status == 200){
                if(data.name == null)
                {
                    Log.e("Response", "updateFcm response if is caleld")
                    _name.value=""
                    _email.value=""
                }
                else{
                    Log.e("Response", "updateFcm response if else else eles  is caleld")
